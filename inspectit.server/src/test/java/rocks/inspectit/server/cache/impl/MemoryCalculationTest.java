...
public class MemoryCalculationTest extends AbstractTestNGLogSupport {
	...
	@Test(invocationCount = 3)
		public void branch() throws IndexingException {
			DefaultData mock = Mockito.mock(DefaultData.class);
			...
			Branch<DefaultData> branch = new Branch<>(new BufferBranchIndexer<>(new ObjectTypeIndexer<>()));
			int size = (int) (Math.random() * 1000000);
			for (long i = 1; i <= size; i++) { // zero key not supported
				branch.put(mock);
			}

			long mockSize = MemoryUtil.deepMemoryUsageOf(mock, VisibilityFilter.ALL);
			long theirSize = MemoryUtil.deepMemoryUsageOf(branch, VisibilityFilter.ALL);
			long ourSize = branch.getComponentSize(objectSizes) + mockSize;
			assertThat("Random branch size " + size, ourSize, is(equalTo(theirSize)));
		}
